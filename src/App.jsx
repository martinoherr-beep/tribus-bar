...
import { 
  ...
  UtensilsCrossed, Zap, CheckCircle, ReceiptText, Printer, Search, CreditCard, Phone
} from 'lucide-react';
...
function App() {
    ...
    return (
      <div className="min-h-screen bg-slate-950 text-white flex flex-col items-center justify-center p-8 font-sans">
        <div className="mb-8 text-center">
          ...
          <h2 className="text-2xl font-black italic uppercase">Acceso Tribu´s Barra</h2>
          <p className="text-slate-500 text-[10px] mt-2 uppercase font-bold tracking-widest">PIN de Seguridad</p>
        </div>
    ...
    <div className="min-h-screen bg-slate-950 text-white flex flex-col items-center justify-center p-8 text-center relative overflow-hidden font-sans">
      ...
      <div className="relative z-10 space-y-12 w-full max-w-lg">
        ...
        <div className="grid gap-4">
          <button onClick={() => { navigator.clipboard.writeText("tribus2026"); alert("Wi-Fi Copiada"); }} className="flex items-center gap-5 bg-slate-800/60 p-5 rounded-3xl border border-slate-700/30 backdrop-blur-sm shadow-xl transition-all active:scale-95"><Wifi className="text-sky-400" size={28} /><div className="text-left font-bold uppercase text-[10px] text-slate-400"><p>Wi-Fi Gratis</p><p className="text-lg text-white font-black">tribus2026</p></div></button>
          <button onClick={() => setView('menu')} className="flex items-center gap-5 bg-orange-600 p-6 rounded-3xl shadow-2xl transition-all active:scale-95"><UtensilsCrossed size={28} /><div className="text-left font-bold uppercase text-[10px] text-orange-200"><p>Menú Digital</p><p className="text-lg text-white font-black uppercase tracking-tight">Ver la carta</p></div></button>
        </div>
        ...
        {ticketParaReimprimir && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/95 backdrop-blur-md print:static print:bg-white print:p-0">
            <div className="bg-white text-black w-full max-w-[280px] p-6 font-mono shadow-2xl relative print-container">
              ...
              <p className="text-[10px] text-center uppercase font-black italic tracking-widest">"¡LA TRIBU TE ESPERA DE VUELTA!"</p>
              <button onClick={() => window.print()} className="mt-8 w-full bg-black text-white py-4 rounded-xl font-black no-print flex items-center justify-center gap-2 transition-all active:scale-95"><Printer size={18}/> Imprimir</button>
            </div>
          </div>
        )}
        <div className="flex-1 no-print">
          <header className="flex justify-between items-center mb-6 border-b border-slate-800 pb-4"><h1 className="text-4xl font-black text-orange-600 italic uppercase tracking-tighter leading-none">BARRA</h1><div className="bg-slate-900 px-3 py-1 rounded-xl text-green-500 text-[10px] font-bold animate-pulse uppercase">● En Vivo</div></header>
            ...
            {filtradosPendientes.map(p => (
              <div key={p.id} className="bg-[#0c111a] border border-slate-800 p-5 rounded-2xl relative shadow-xl flex flex-col justify-between group">
                ...
                <div>
                  ...
                  {String(p.mesa).startsWith("TEL:") && <p className="text-blue-500 font-black text-xs mb-3 tracking-widest uppercase">{p.mesa}</p>}
            ...
            <div className="space-y-3 max-h-[450px] overflow-y-auto no-scrollbar mb-4">
              {historialParaMostrar.map((hc) => (
                <div key={hc.id} className="group p-3 bg-[#05070a] rounded-xl border border-slate-700 flex items-center justify-between hover:border-orange-500 transition-all">
                  ...
                  <button onClick={async (e) => { e.stopPropagation(); if(window.confirm("¿Borrar de caja?")) await deleteDoc(doc(db, "historial_tickets", hc.id)); }} className="ml-2 p-1.5 text-slate-700 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all"><Trash2 size={14}/></button>
                </div>
              ))}
            </div>
  ...
  // --- VISTA MENÚ ---
  const menuFiltrado = MENU_LOCAL.filter(p => {
    ...
    <div className="min-h-screen bg-slate-900 pb-32 text-slate-100 flex flex-col items-center font-sans">
      <header className="bg-slate-950/95 backdrop-blur-md sticky top-0 z-40 w-full border-b border-slate-800 px-4 py-3">
        <div className="max-w-6xl mx-auto flex flex-col gap-3">
          ...
          <div className="flex gap-2 overflow-x-auto no-scrollbar">{CATEGORIAS.map(c => (<button key={c} onClick={() => { setCatSeleccionada(c); setSubCatSeleccionada("Todas"); }} className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase whitespace-nowrap border ${catSeleccionada === c ? 'bg-orange-600 text-white border-orange-500' : 'bg-slate-900 text-slate-400 border-slate-800'}`}>{c}</button>))}</div>
          {catSeleccionada === "Cerveza" && (<div className="flex gap-2 overflow-x-auto no-scrollbar pt-1 border-t border-slate-800/50">{["Todas", "Media", "Caguama"].map(sc => (<button key={sc} onClick={() => setSubCatSeleccionada(sc)} className={`px-3 py-1 rounded-lg text-[9px] font-bold uppercase transition-all ${subCatSeleccionada === sc ? 'text-sky-400 border-sky-400/30 bg-sky-900/20' : 'text-slate-500'}`}>{sc}</button>))}</div>)}
        </div>
      </header>
      ...
      <div className={`fixed inset-0 z-[100] flex items-center justify-center p-4 transition-all ${verModalTelefono ? 'visible opacity-100' : 'invisible opacity-0'}`}>
        ...
        <div className="relative bg-slate-900 border border-slate-800 w-full max-w-[320px] rounded-[2.5rem] p-8 shadow-2xl text-center flex flex-col items-center">
           <div className="bg-green-600/20 w-16 h-16 rounded-3xl flex items-center justify-center mb-6 text-green-500"><Phone size={32}/></div>
           <h2 className="text-2xl font-black italic uppercase tracking-tighter text-white mb-2">Tu Teléfono</h2>
           ...
           <div className="w-full bg-slate-950 border border-slate-800 rounded-2xl py-4 mb-8 text-2xl font-black text-green-500 tracking-[0.2em] min-h-[64px] flex items-center justify-center">
             {telefonoInput || <span className="opacity-20 text-slate-600">0000000000</span>}
           </div>

           <div className="grid grid-cols-3 gap-3 w-full mb-8">
             {[1,2,3,4,5,6,7,8,9].map(n => (
               <button key={n} onClick={() => telefonoInput.length < 10 && setTelefonoInput(telefonoInput + n)} className="h-14 rounded-2xl bg-slate-800/50 border border-slate-700 text-xl font-black hover:bg-slate-700 transition-all active:scale-90">{n}</button>
             ))}
             <button onClick={() => setTelefonoInput("")} className="h-14 rounded-2xl bg-red-900/20 text-red-500 flex items-center justify-center active:scale-90"><Trash2 size={20}/></button>
           ...
           <button 
            ...
            onClick={() => procesarEnvio(`TEL: ${telefonoInput}`)}
            className={`w-full py-4 rounded-2xl font-black uppercase tracking-widest transition-all active:scale-95 ${telefonoInput.length >= 10 ? 'bg-green-600 shadow-lg shadow-green-900/20' : 'bg-slate-800 text-slate-600 cursor-not-allowed'}`}
           >
             Enviar Pedido
           </button>
           <button onClick={() => setVerModalTelefono(false)} className="mt-4 text-slate-500 text-[10px] font-bold uppercase tracking-widest border-b border-transparent hover:border-slate-500">Cancelar</button>
        </div>
      </div>

      {carrito.length > 0 && !verCarrito && !verModalTelefono && (
        <div className="fixed bottom-6 left-0 right-0 px-6 z-50 flex justify-center no-print"><button onClick={() => setVerCarrito(true)} className="w-full max-w-lg bg-orange-600 text-white py-4 rounded-2xl font-black flex justify-between px-8 shadow-2xl active:scale-95 transition-all"><span className="text-[10px] uppercase font-bold tracking-widest text-white">🛒 MI PEDIDO ({carrito.reduce((a,b)=>a+b.cantidad,0)})</span><span className="font-black text-xl italic text-white">${totalCarrito}</span></button></div>
      )}
    </div>
  );
}
