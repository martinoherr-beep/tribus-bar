...
const LAT_BAR = 70.930772061715487;        
const LON_BAR = -200.66753182968978;      
const RADIO_MAXIMO_METROS = 30;  // Distancia máxima en metros para considerarlo "en el bar"
...
function App() {
  ...
  const [verModalPin, setVerModalPin] = useState(false);
...
const obtenerPrecioItem = (item) => {
 ...
 useEffect(() => {
  ...
  query(collection(db, "pedidos"), where("estado", "in", ["pendiente", "en_preparacion", "entregado"])), 
  (snapshot) => {
    const data = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
    setPedidosBarra(data);
    
    // Si la comanda del cliente local sigue activa, la buscamos
    const comandaIdGuardada = localStorage.getItem("tribu_comanda_id");
    let pedidoMesa = null;
    if (comandaIdGuardada) {
...
const intentarEnviar = () => {
  ...
  // Forzamos el string en mayúsculas para evitar errores de dedo ("t" o "b")
  const mesaLimpia = mesa ? String(mesa).toUpperCase().trim() : "";
  
  // Condición infalible: Si no hay mesa, o es "T", o es "B", ES DE CASA (EXTERNO)
  const esDeCasa = !mesaLimpia || mesaLimpia === "T" || mesaLimpia === "B";

  if (usuarioLogueado) {
    // Si ya inició sesión con su cuenta de la Tribu, tiene teléfono guardado; pasa directo
    procesarEnvio(mesa);
  } else if (esDeCasa) {
    // 📲 Si es de casa y no está logueado, forzar pantalla de teléfono
    setVerModalTelefono(true);
  } else {
    // Si es una mesa física numérica (ej: "12"), pasa directo
    procesarEnvio(mesa);
  }
};
    ...
    >
      ...
      {p.pagoInformado && (
        <div className="bg-blue-600/20 border border-blue-500 text-blue-400 p-2 rounded-xl mb-2 flex items-center justify-between animate-pulse">
          ...
          <span className="text-[8px] font-black bg-blue-500 text-black px-1.5 py-0.5 rounded uppercase">
            Verificar Banco
          </span>
        </div>
      )}
      {/* 🔔 ALERTA 2: MESA FÍSICA PIDE LA CUENTA */}
      {p.solicitaCuenta && (
        <div className="bg-amber-500/20 border border-amber-500 text-amber-400 p-2.5 rounded-xl mb-2 flex items-center justify-between animate-pulse shadow-lg shadow-amber-950/40">
          <span className="text-[10px] font-black uppercase tracking-wider flex items-center gap-1">
            🔔 ¡MESA {p.mesa} SOLICITA CUENTA!
          </span>
          <span className="text-[8px] font-black bg-amber-500 text-black px-2 py-0.5 rounded uppercase">
            Llevar Ticket
          </span>
        </div>
      )}
